...
// ── Props ─────────────────────────────────────────────────────────────────────
type Props = {
    event: (Event & { media?: { id: number; file_path: string; is_primary: boolean }[] }) | null;
    villages: Pick<Village, 'id' | 'name'>[];
    destinations: Pick<Destination, 'id' | 'name' | 'village_id'>[];
    isAdmin: boolean;
};

export default function EventForm({ event, villages, destinations, isAdmin }: Props) {
    ...
    useEffect(() => {
        if (isAdmin && data.village_id) {
            setFilteredDestinations(destinations.filter((d) => d.village_id === data.village_id));
    ...
    const editor = useEditor({
        extensions: [
            StarterKit,
            Placeholder.configure({ placeholder: 'Deskripsikan event ini: kegiatan, jadwal, informasi penting...' }),
        ],
    ...
    const validate = (): boolean => {
        ...
        if (data.end_date && data.start_date && data.end_date < data.start_date) {
            setError('end_date' as keyof typeof errors, 'Tanggal selesai harus sama atau setelah tanggal mulai.');
            return false;
        }
    ...
    const submit = (e: React.FormEvent, publishNow = false) => {
        ...
        if (isEditing) {
            ...
            router.post('/admin/events', finalData as unknown as Record<string, unknown>, options);
        }
    };
            ...
            <form onSubmit={submit} className="flex flex-col gap-6 p-6">
                ...
                <div>
                    <h1 className="font-display text-2xl font-semibold text-[oklch(0.24_0.05_145)]">
                        {isEditing ? `Edit: ${event.title}` : 'Tambah Event Baru'}
                    </h1>
                    <p className="mt-0.5 text-sm text-[oklch(0.48_0.01_85)]">Lengkapi semua informasi event.</p>
                </div>
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        ...
                        <CardDescription>Judul, desa penyelenggara, dan lokasi event.</CardDescription>
                    </CardHeader>
                    <CardContent className="flex flex-col gap-5">
                        ...
                        <div className="grid gap-5 sm:grid-cols-2">
                            <div className="flex flex-col gap-1.5">
                                <Label htmlFor="village_id">Desa Penyelenggara <span className="text-destructive">*</span></Label>
                                {isAdmin ? (
                            ...
                            <div className="flex flex-col gap-1.5">
                                <Label htmlFor="destination_id">Lokasi Destinasi (opsional)</Label>
                                <Select
                                    value={data.destination_id ? String(data.destination_id) : 'none'}
                                    onValueChange={(v) => setData('destination_id', v === 'none' ? null : Number(v))}
                                >
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        <CardTitle className="font-display text-lg text-[oklch(0.24_0.05_145)]">Waktu Pelaksanaan</CardTitle>
                        <CardDescription>Tanggal dan jam mulai/selesai event.</CardDescription>
                    </CardHeader>
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        <CardTitle className="font-display text-lg text-[oklch(0.24_0.05_145)]">Deskripsi Event</CardTitle>
                        <CardDescription>Informasi lengkap tentang event, kegiatan, dan hal penting lainnya.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        <CardTitle className="font-display text-lg text-[oklch(0.24_0.05_145)]">Detail & Kontak</CardTitle>
                        <CardDescription>Informasi tiket, penyelenggara, dan kontak.</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-5 sm:grid-cols-2">
                        <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex flex-col gap-1.5">
                            <Label htmlFor="contact_person">Kontak Person (WA/HP)</Label>
                            <Input
                                id="contact_person"
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        <CardTitle className="font-display text-lg text-[oklch(0.24_0.05_145)]">Foto Event</CardTitle>
                        <CardDescription>Upload foto poster atau dokumentasi event.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <ImageUploader existing={event?.media ?? []} onChange={handleMediaChange} />
                    </CardContent>
                </Card>
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    ...
                    <CardContent className="flex flex-col gap-5">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            />
                            <p className="text-xs text-[oklch(0.48_0.01_85)]">Kosongkan untuk menggunakan URL halaman event ini.</p>
