...
// ── Constants ─────────────────────────────────────────────────────────────────
const FACILITIES_OPTIONS = ['Parkir', 'Toilet', 'Mushola', 'Warung/Kantin', 'WiFi', 'Gazebo', 'Area Bermain', 'Penginapan', 'Pemandu Wisata', 'Souvenir'];
...
const destinationSchema = z.object({
    ...
    village_id: z.number({ message: 'Desa wajib dipilih' }).int().positive('Desa wajib dipilih'),
    category: z.enum(['alam', 'budaya', 'buatan'], { message: 'Kategori wajib dipilih' }),
    ticket_price: z.number({ message: 'Harga harus berupa angka' }).min(0, 'Harga tidak boleh negatif'),
...
// ── Props ─────────────────────────────────────────────────────────────────────
type Props = {
    destination: (Destination & { media?: { id: number; file_path: string; is_primary: boolean }[] }) | null;
    villages: Pick<Village, 'id' | 'name'>[];
    isAdmin: boolean;
};

export default function DestinationForm({ destination, villages, isAdmin }: Props) {
    ...
    >({
        ...
        open_time: destination?.open_time ? destination.open_time.substring(0, 5) : '',
        close_time: destination?.close_time ? destination.close_time.substring(0, 5) : '',
    ...
    const editor = useEditor({
        extensions: [
            StarterKit,
            Placeholder.configure({ placeholder: 'Deskripsikan destinasi wisata ini: keindahan, keunikan, tips berkunjung...' }),
        ],
    ...
    const submit = (e: React.FormEvent, publishNow = false) => {
        ...
        if (isEditing) {
            ...
            router.post('/admin/destinations', finalData as unknown as Record<string, unknown>, options);
        }
    };

    const handleMediaChange = useCallback(
        (files: File[], deletedIds: number[], primaryId: number | null) => {
            setData((prev) => ({ ...prev, images: files, deleted_media_ids: deletedIds, primary_media_id: primaryId }));
        },
        [setData],
    );

    const toggleDay = (day: string) => {
        const current = data.operational_days ? data.operational_days.split(', ').filter(Boolean) : [];
    ...
    const selectedDays = data.operational_days ? data.operational_days.split(', ').filter(Boolean) : [];

    return (
        <>
            <Head title={isEditing ? `Edit ${destination.name}` : 'Tambah Destinasi'} />
            ...
            <form onSubmit={submit} className="flex flex-col gap-6 p-6">
                ...
                <div>
                    <h1 className="font-display text-2xl font-semibold text-[oklch(0.24_0.05_145)]">
                        {isEditing ? `Edit: ${destination.name}` : 'Tambah Destinasi Baru'}
                    </h1>
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        ...
                        <CardDescription>Identitas utama destinasi wisata.</CardDescription>
                    </CardHeader>
                    <CardContent className="flex flex-col gap-5">
                        <div className="flex flex-col gap-1.5">
                            <Label htmlFor="name">Nama Destinasi <span className="text-destructive">*</span></Label>
                            <Input
                                id="name"
                                value={data.name}
                        ...
                        <div className="grid gap-5 sm:grid-cols-2">
                            {/* Village selector */}
                            <div className="flex flex-col gap-1.5">
                                <Label htmlFor="village_id">Desa <span className="text-destructive">*</span></Label>
                                {isAdmin ? (
                                    <Select value={String(data.village_id)} onValueChange={(v) => setData('village_id', Number(v))}>
                                        <SelectTrigger id="village_id" className={errors.village_id ? 'border-destructive' : ''}>
                                    ...
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <p className="text-xs text-[oklch(0.48_0.01_85)]">Desa penyelenggara (otomatis desa Anda).</p>
                                    </div>
                                )}
                                {errors.village_id && <p className="text-xs text-destructive">{errors.village_id}</p>}
                            </div>

                            {/* Category */}
                            <div className="flex flex-col gap-1.5">
                                <Label htmlFor="category">Kategori <span className="text-destructive">*</span></Label>
                                <Select value={data.category} onValueChange={(v) => setData('category', v as typeof data.category)}>
                                    <SelectTrigger id="category" className={errors.category ? 'border-destructive' : ''}>
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="alam">🌲 Wisata Alam</SelectItem>
                                        <SelectItem value="budaya">🏛️ Wisata Budaya & Religi</SelectItem>
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        <CardTitle className="font-display text-lg text-[oklch(0.24_0.05_145)]">Deskripsi</CardTitle>
                        <CardDescription>Ceritakan tentang destinasi: keindahan, keunikan, dan tips berkunjung.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        <CardTitle className="font-display text-lg text-[oklch(0.24_0.05_145)]">Tiket & Operasional</CardTitle>
                        <CardDescription>Informasi harga tiket, jam buka, dan fasilitas.</CardDescription>
                    </CardHeader>
                    <CardContent className="flex flex-col gap-6">
                        <div className="grid gap-5 sm:grid-cols-2">
                            <div className="flex flex-col gap-1.5">
                                <Label htmlFor="ticket_price">Harga Tiket (Rp) <span className="text-destructive">*</span></Label>
                                <Input
                                    id="ticket_price"
                                    type="number"
                                    min={0}
                                    ...
                                    onChange={(e) => setData('ticket_price', parseFloat(e.target.value) || 0)}
                                    placeholder="0 = gratis"
                                    className={errors.ticket_price ? 'border-destructive' : ''}
                                />
                                <p className="text-xs text-[oklch(0.48_0.01_85)]">Isi 0 jika gratis.</p>
                                {errors.ticket_price && <p className="text-xs text-destructive">{errors.ticket_price}</p>}
                            </div>
                            <div className="flex flex-col gap-1.5">
                                <Label htmlFor="ticket_info">Keterangan Tiket</Label>
                                <Input
                                    id="ticket_info"
                        ...
                        <div className="grid gap-5 sm:grid-cols-2">
                            <div className="flex flex-col gap-1.5">
                                ...
                                <TimePicker
                                    ...
                                    onChange={(val) => setData('open_time', val)}
                                />
                            </div>
                            <div className="flex flex-col gap-1.5">
                                ...
                                <TimePicker
                                    ...
                                    onChange={(val) => setData('close_time', val)}
                                />
                            </div>
                        </div>
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        <CardTitle className="font-display text-lg text-[oklch(0.24_0.05_145)]">Lokasi</CardTitle>
                        <CardDescription>
                            <span className="flex items-center gap-1.5">
                                <MapPin className="h-4 w-4 text-[oklch(0.38_0.08_145)]" />
                                Seret pin atau klik peta untuk menentukan koordinat.
                            </span>
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="flex flex-col gap-5">
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    <CardHeader>
                        <CardTitle className="font-display text-lg text-[oklch(0.24_0.05_145)]">Foto Destinasi</CardTitle>
                        <CardDescription>Upload foto-foto terbaik destinasi. Klik foto untuk jadikan cover.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <ImageUploader existing={destination?.media ?? []} onChange={handleMediaChange} />
                    </CardContent>
                </Card>
                ...
                <Card className="border-[oklch(0.22_0.01_85/8%)] shadow-none">
                    ...
                    <CardContent className="flex flex-col gap-5">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            />
                            <p className="text-xs text-[oklch(0.48_0.01_85)]">Kosongkan untuk menggunakan URL halaman destinasi ini.</p>
                            {errors.qr_code_target && <p className="text-xs text-destructive">{errors.qr_code_target}</p>}
                        </div>
                    </CardContent>
                </Card>
                ...
                <div className="sticky bottom-0 -mx-6 flex items-center justify-between border-t border-[oklch(0.22_0.01_85/8%)] bg-white/90 px-6 py-4 backdrop-blur-sm">
                    ...
                    <div className="flex gap-3">
                        ...
                        <Button type="submit" disabled={processing} className="bg-[oklch(0.38_0.08_145)] hover:bg-[oklch(0.24_0.05_145)]">
                            ...
                            {isEditing ? 'Simpan Perubahan' : 'Tambah Destinasi'}
                        </Button>
                    </div>
                </div>
            </form>
...
DestinationForm.layout = (page: React.ReactNode & { props: Props }) => {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: destination ? `Edit: ${destination.name}` : 'Tambah Destinasi', href: '#' },
    ];
